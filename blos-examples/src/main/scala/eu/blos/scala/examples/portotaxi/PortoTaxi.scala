...
object PortoTaxi {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for( h <- Range(0,24) ) {
      ...
      //numDistinctElementsLongTrips = 0L
      //numDistinctElementsShortTrips = 0L

      numDistinctElementsLongTrips = skeching(sketchLongTrips, new CSVIterator(new FileReader(new File(filename)), "\t"), inputspaceNormalizer, h, h+1, TRIPTYPE_LONG,bloomLongTrips, numDistinctElementsLongTrips)
      numDistinctElementsShortTrips = skeching(sketchShortTrips, new CSVIterator(new FileReader(new File(filename)), "\t"), inputspaceNormalizer, h, h+1, TRIPTYPE_SHORT, bloomShortTrips, numDistinctElementsShortTrips)
  ...
  def skeching(sketch : CMSketch, dataset : CSVIterator, normalizer : InputSpaceNormalizer[DoubleVector], hourFrom : Int, hourTo : Int, filter_tripType : Int, bloom : BloomFilter, distinctCounterIn : Long) = {
