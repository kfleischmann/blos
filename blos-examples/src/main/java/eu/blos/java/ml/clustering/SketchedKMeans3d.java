...
public class SketchedKMeans3d {
	...
	public static List<Tuple3<Double,Double,Double>> dataset = new ArrayList<>();
	public static Tuple3<Double,Double,Double>[] centroids;
	...
	public static void main(String[] args) throws Exception {
		...
		centroids = new Tuple3[numCentroids];
			...
			if (!cmd.hasOption("output")) {
				...
				for (int k = 1; k < sketch.getHeavyHitters().getHeapArray().length; k++) {
					...
					if (topK != null) {
						...
						Tuple3<Double, Double,Double> d = new Tuple3<>(Double.parseDouble(values[0]), Double.parseDouble(values[1]), Double.parseDouble(values[2]) );
	...
	/**
	 * read 3d points from input source
	 * possible
	 * - stdin
	 * - file
	 * @param is
	...
	public static void buildSketches( InputStreamReader is ) {
		...
		try (BufferedReader br = new BufferedReader( is )) {
			...
			while ((line = br.readLine()) != null) {
				...
				Tuple3<Double,Double,Double> Xi = new Tuple3<>(  normalizer.normalize(Double.parseDouble(values[0])), normalizer.normalize(Double.parseDouble(values[1])),normalizer.normalize(Double.parseDouble(values[2])) );
	 ...
	 * @param centroids
	 */
	public static void initCentroidsRandomly(Tuple3<Double,Double,Double>[] centroids){
		Random r = new Random();
		for( int c=0; c < centroids.length; c++ ) {
			// use zero - one normalizer and put the
			centroids[c] = new Tuple3<>( normalizer.getRandom(), normalizer.getRandom(),normalizer.getRandom() );
	 ...
	 * @throws Exception
	 */
	public static void initCentroidsHH(Tuple3<Double,Double,Double>[] centroids) throws Exception {
		for( int c=0; c < centroids.length; c++ ) {
			CMEstimate topK = (CMEstimate) sketch.getHeavyHitters().getHeapArray()[c+1];
			if (topK != null) {
				String[] values = topK.key().replaceAll("[^-0-9,.E]", "").split(",");
				centroids[c] =  new Tuple3<>(Double.parseDouble(values[0]), Double.parseDouble(values[1]), Double.parseDouble(values[2]));
			} else {
	 ...
	 * assuming the first field is an index value
	 * @param centroids
	 * @param file
	 */
	public static void initCentroidsbyFile(Tuple3<Double,Double,Double>[] centroids, String file ){
		try {
			...
			while ((line = br.readLine()) != null) {
				String[] values = line.split(" ");
				centroids[c] = new Tuple3<>(Double.parseDouble(values[1]), Double.parseDouble(values[2]), Double.parseDouble(values[3]));
	 ...
	 * learn the model
	 */
	public static void learnByEnum(Tuple3<Double,Double,Double>[] centroids) throws Exception {
	 ...
	 * learn the model
	 */
	public static void learnByHH(Tuple3<Double,Double,Double>[] centroids) throws Exception {
	 ...
	 * @param centroids
	 */
	public static void updateClusterCentroidsWithHeavyHitters( Tuple3<Double,Double,Double>[] centroids ){
		long freq;
		String lookup;
		long inputSpace=0;

		Tuple3<Double,Double,Double>[] sums = new Tuple3[centroids.length];
		for( int l=0; l < sums.length; l++) sums[l] = new Tuple3<>(0.0,0.0,0.0);
		...
		for( int k=0; k < sketch.getHeavyHitters().getHeapArray().length-1; k++ ){
			...
			if(topK!=null) {
				String[] values = topK.key().replaceAll("[^-0-9,.E]","").split(",");
				Tuple3<Double,Double,Double> value = new Tuple3<>(Double.parseDouble(values[0]), Double.parseDouble(values[1]), Double.parseDouble(values[2]) ) ;
				...
				if(freq>0) {
					...
					for (int i = 0; i < centroids.length; i++) {
						double d = Math.sqrt(
								...
								(value.f2 - centroids[i].f2) * (value.f2 - centroids[i].f2) 		);
						if( d < currDistance){
							ibestCentroid = i;
							currDistance= d;
						}
					...
					sums[ibestCentroid].f2 += value.f2*freq;
				}//if
			}//if
		}//for
		// update centroids
		for (int i = 0; i < centroids.length; i++) {
			...
			centroids[i].f2 = sums[i].f2 / counts[i];
	 ...
	 * @param centroids
	 */
	public static void updateClusterCentroidsWithEnumeration( Tuple3<Double,Double,Double>[] centroids ) throws FileNotFoundException {
		long freq;
		String lookup;
		long inputSpace=0;
		...
		for (double y = (double) normalizer.getMax(); y >= (double) normalizer.getMin(); y -= (double) normalizer.getStep()) {
			for (double x = (double) normalizer.getMin(); x <= (double) normalizer.getMax(); x += (double) normalizer.getStep()) {
				for (double z = (double) normalizer.getMin(); z <= (double) normalizer.getMax(); z += (double) normalizer.getStep()) {
					Tuple3<Double, Double, Double> value = new Tuple3<>(normalizer.normalize(x), normalizer.normalize(y), normalizer.normalize(z));
	...
	public static void reconstructInputSpace() throws FileNotFoundException {
		...
		Tuple3<Integer,Integer,Integer> pos = new Tuple3<Integer,Integer,Integer>();
		...
		for (double y = (double) normalizer.getMax(); y >= (double) normalizer.getMin(); y -= (double) normalizer.getStep()) {
			for (double x = (double) normalizer.getMin(); x <= (double) normalizer.getMax(); x += (double) normalizer.getStep()) {
				for (double z = (double) normalizer.getMin(); z <= (double) normalizer.getMax(); z += (double) normalizer.getStep()) {
					...
					// enumerate
					Tuple3<Double, Double, Double> value = new Tuple3<>(normalizer.normalize(x), normalizer.normalize(y), normalizer.normalize(z));
					lookup = "(" + normalizer.normalize(x) + "," + normalizer.normalize(y) + "," + normalizer.normalize(z) + ")";

					freq = sketch.get(lookup);

					out_enum.println(StringUtils.join(Arrays.asList(pos.f0, pos.f1, pos.f2), " ") + " "+freq);

					// hh
					if (hh.containsKey(lookup)) {
						freq = hh.get(lookup);
						out_hh.println(StringUtils.join(Arrays.asList(pos.f0, pos.f1, pos.f2), " ") + " "+freq);
					} else {
						out_hh.println(StringUtils.join(Arrays.asList(pos.f0, pos.f1, pos.f2), " ") + " "+0);
					}
